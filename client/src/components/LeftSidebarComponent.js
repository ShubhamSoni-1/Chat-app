...
function LeftSidebarComponent({
  ...
  {
    /*useEffect(() => {
    socket.emit(SocketEvent.CHANGE_NAME, name, color);
  }, [name]);*/
  }
  ...
  return (
    <Fragment>
      <div className={`left-sidebar-container ${isSidebarHidden && `hidden`}`} onClick={() => setIsSidebarHidden(true)}>
        <div className={`left-sidebar ${isSidebarHidden && `hidden`}`} onClick={(e) => e.stopPropagation()}>
